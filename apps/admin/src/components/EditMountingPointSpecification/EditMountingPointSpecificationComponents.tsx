...
export const EditMountingPointSpecificationComponents = () => {
  ...
  const filteredAddComponent = Object.keys(componentsSnap.components).filter(
    (componentSpecId) => {
      ...
      const isAdded =
        mountingPoint.mountableComponents.includes(componentSpecId);

      return isIncluded && !isAdded;
    }
  );

  const addComponent = (mountedComponentId: string) => {
    ...
    editableMountingPoint.mountableComponents.push(mountedComponentId);
  };
