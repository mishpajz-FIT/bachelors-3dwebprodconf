...
import { AddMountingPoint } from "../Add/AddMountingPoint.tsx";
...
export const EditComponentSpecificationMountingPoints = () => {
  ...
  return (
    <div>
      <h3 className="section-heading">Mounting Points</h3>
      <div className="mx-4 rounded-md bg-slate-50 p-1 outline outline-1 outline-slate-100 dark:bg-zinc-800 dark:outline-zinc-800">
        ...
        >
          <label htmlFor="mounting-point-search" className="sr-only">
            Search mounting points
          </label>
          <div className="relative">
            ...
            <input
              type="search"
              id="mounting-point-search"
              className="field block w-full bg-transparent ps-10"
              placeholder="Search mounting points..."
              onChange={(e) => {
        ...
        <ul className="flex max-h-48 min-h-24 shrink-0 flex-col items-center justify-start overflow-x-clip overflow-y-scroll overscroll-y-contain p-2">
          {filteredMountingPoints.length === 0 ? (
            <li className="p-4 text-center text-sm text-gray-900 dark:text-gray-400">
              No mounting points
            </li>
          ) : (
            filteredMountingPoints.map((mountingPointId) => (
              <li key={mountingPointId} className="tile m-1 w-full rounded-lg">
                <button
                  onClick={() => {
        ...
        <div className="flex w-full flex-row justify-end border-t border-gray-300 dark:border-zinc-700">
          ...
          <Popup isOpen={isOpenAdd} onClose={() => setOpenAdd(false)}>
            <AddMountingPoint onClose={() => setOpenAdd(false)} />
          </Popup>
        </div>
      </div>
      <Side
        isOpen={editorValuesSnap.selectedMountingPoint !== undefined}
        larger={true}
      >
